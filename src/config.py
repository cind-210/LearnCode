...
MINI_CODE_DIR = Path(
    os.environ.get("MINI_CODE_HOME")
    or os.path.join(os.path.expanduser("~"), ".mini-code")
).resolve()
...
async def read_mcp_tokens_file(file_path: Optional[Path] = None) -> dict[str, str]:
    path = file_path or MINI_CODE_MCP_TOKENS_PATH
    try:
...
async def load_effective_settings() -> MiniCodeSettings:
    ...
    mini_code = await read_settings_file(MINI_CODE_SETTINGS_PATH)
...
async def save_mini_code_settings(updates: MiniCodeSettings) -> None:
    ...
    _write_json_file(MINI_CODE_SETTINGS_PATH, {
        "env": merged.env,
        "model": merged.model,
...
async def load_runtime_config() -> RuntimeConfig:
    ...
    base_url = (
        os.environ.get("MINI_CODE_ANTHROPIC_BASE_URL")
        or os.environ.get("MINI_CODE_OPENAI_BASE_URL")
        or str(env.get("ANTHROPIC_BASE_URL", "")).strip()
    ) or "https://api.anthropic.com/v1/messages"

    provider = "anthropic"
    if (os.environ.get("MINI_CODE_OPENAI_BASE_URL", "") and
            not os.environ.get("MINI_CODE_ANTHROPIC_BASE_URL", "")):
        provider = "openai"

    auth_token = (
        os.environ.get("MINI_CODE_AUTH_TOKEN")
        or str(env.get("ANTHROPIC_AUTH_TOKEN", "")).strip()
    ) or None

    api_key = (
        os.environ.get("MINI_CODE_API_KEY")
    ...
    if not model:
        raise RuntimeError("No model configured. Set ~/.mini-code/settings.json or MINI_CODE_MODEL env.")
    ...
    return RuntimeConfig(
        ...
        source_summary=f"config: {MINI_CODE_SETTINGS_PATH} > {CLAUDE_SETTINGS_PATH} > process.env",
    )